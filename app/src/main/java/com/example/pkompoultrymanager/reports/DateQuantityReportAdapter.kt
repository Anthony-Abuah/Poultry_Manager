...
class DateQuantityReportAdapter(number: Int): RecyclerView.Adapter<DateQuantityReportAdapter.MyViewHolder>() {
    ...
    private var quantity1 = emptyList<Double>()
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.itemView.apply {
            ...
            val stringNumberOfEggsRemaining = if (numberOfEggsRemaining==1)"$numberOfEggsRemaining egg" else "$numberOfEggsRemaining eggs"
            val totalEggsAddedString = if (numberOfEggCrates < 1) {stringNumberOfEggsRemaining} else if(numberOfEggCrates == 1) {"$numberOfEggCrates crate and $stringNumberOfEggsRemaining"} else {"$numberOfEggCrates crates and $stringNumberOfEggsRemaining"}

            holder.itemDate.text = currentDate
            holder.itemQuantity.text = totalEggsAddedString
        }
