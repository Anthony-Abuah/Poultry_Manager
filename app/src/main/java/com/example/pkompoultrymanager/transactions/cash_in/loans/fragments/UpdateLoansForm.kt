...
class UpdateLoansForm : Fragment() {
    ...
    override fun onCreateView(
        ...
        val theLoansId = arguments?.getInt("loansId")
        val theDate = arguments?.getString("date")
        val theLender = arguments?.getString("lender")
        val theLoanAmount = arguments?.getString("loanAmount")
        ...
        etLoanAmount.setText(theLoanAmount)
    ...
    private fun insertDataToDatabase(
        ...
        if (checkIfEmpty(date)) {
            ...
            val loanDate = newDate?.time?.let { java.sql.Date(it)}

            if (loanDate != null) {
                mLoansViewModel.updateLoansInfo(loanId, loanDate, lender, loanAmount.toDouble(), transactionId, shortNotes)
            }
