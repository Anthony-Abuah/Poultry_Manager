...
import com.example.pkompoultrymanager.transactions.cash_in.investment.InvestmentAdapter
...
class InvestmentList : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_investment_list, container, false)
        ...
        val dateFilter = view.findViewById<ImageButton>(R.id.ibDateFilter_InvestmentList)


        val investmentList = view.findViewById<RecyclerView>(R.id.rvInvestmentList)
        val adapter = InvestmentAdapter(currency!!)
        investmentList.adapter = adapter
        investmentList.layoutManager= LinearLayoutManager(requireContext())
        mInvestmentViewModel = ViewModelProvider(this)[InvestmentViewModel::class.java]

        mInvestmentViewModel = ViewModelProvider(this)[InvestmentViewModel::class.java]
        mInvestmentViewModel.displayAllInvestments.observe(viewLifecycleOwner
        ) { investment -> adapter.setData(investment) }
        ...
        dateFilter.setOnClickListener {
            ...
            popupDateFilterMenu.setOnMenuItemClickListener {
                when (it.itemId) {
                    R.id.miAllTime -> {
                        mInvestmentViewModel.displayAllInvestments.observe(
                            viewLifecycleOwner
                        )
                        { investment -> adapter.setData(investment) }
                        true
                    ...
                    R.id.miToday -> {
                        mInvestmentViewModel.displayInvestmentByDate(
                            numberOfList,
                            thePreviousDay!!,
                            theCurrentDate!!
                            ...
                            setPositiveButton("Ok") { dialog, where ->
                                ...
                                if (checkIfEmpty(tvDatePickerFrom.text.toString())) {
                                    ...
                                    if (theFirstDate != null) { if (theLastDate != null) {
                                        mInvestmentViewModel.displayInvestmentByDate(listNumber, theFirstDate, theLastDate).observe(viewLifecycleOwner)
                                        { investment -> adapter.setData(investment) } } }
                                }
                            }
        ...
        adapter.setOnItemClickListener(object : InvestmentAdapter.OnItemClickListener {
            val myFormat = "dd-MM-yyyy"
            val simpleDateFormat = SimpleDateFormat(myFormat, Locale.UK)
            override fun onDeleteInvestmentClickListener(investment: Investment, position: Int) {
                val popupMenu = PopupMenu(requireContext(), investmentList[position].findViewById(R.id.ivUpdate_Delete_Menu_LoansAndInvestmentLayout))
                ...
                popupMenu.setOnMenuItemClickListener {
                    when (it.itemId) {
                        R.id.miDelete -> {
                            ...
                            with(builder) {
                                setTitle("Are you sure you want to permanently delete this investment?")

                                setNegativeButton("No") { dialog, which ->
                                    investmentList.adapter = adapter
                                    investmentList.layoutManager =
                                        LinearLayoutManager(requireContext())
                                    Toast.makeText(
                                        requireContext(), "Investment not removed",
                                        Toast.LENGTH_LONG
                                    ).show()
                                }
                                setPositiveButton("Yes") { dialog, where ->
                                    mInvestmentViewModel.delete(investment)
                                    Toast.makeText(
                                        requireContext(), "Investment removed",
                                        Toast.LENGTH_LONG
                                    ).show()
                                }
                                setView(dialogLayout)
                        ...
                        R.id.miUpdate -> {
                            ...
                            val investmentId = investment.InvestmentId.toString()
                            val date = simpleDateFormat.format(investment.Date)
                            val investor = investment.Investor
                            val amountInvested = investment.AmountInvested.toString()
                            val transactionId = investment.TransactionID
                            val shortNotes = investment.shortNotes
                            communicator = activity as CashInCommunicator
                            communicator.passInvestment(
                                investmentId,
                                date,
                                investor,
                                amountInvested,
            ...
            override fun onItemClickListener(investment: Investment) {
                val investmentId = investment.InvestmentId.toString()
                val date = simpleDateFormat.format(investment.Date)
                val investor = investment.Investor
                val amountInvested = investment.AmountInvested.toString()
        ...
        val addInvestment = view.findViewById<FloatingActionButton>(R.id.fabAddInvestment)
        addInvestment.setOnClickListener {
