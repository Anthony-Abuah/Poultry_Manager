...
class UpdateLoanRepaymentForm : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_update_loans_form, container, false)

        mLoanRepaymentViewModel = ViewModelProvider(this)[LoanRepaymentViewModel::class.java]

        // Declaring the various Views
        //display views
        val tvEnterDate = view.findViewById<TextView>(R.id.tvEnterDate_UpdateLoanRepaymentForm)
        val tvAmountRepaid = view.findViewById<TextView>(R.id.tvAmountRepaid_UpdateLoanRepaymentForm)
        val tvSelectLender = view.findViewById<TextView>(R.id.tvLender_UpdateLoanRepaymentForm)
        val tvTransactionId = view.findViewById<TextView>(R.id.tvTransactionId_UpdateLoanRepaymentForm)
        val tvSelectPaymentMethod = view.findViewById<TextView>(R.id.tvPaymentMethod_UpdateLoanRepaymentForm)
        val save = view.findViewById<Button>(R.id.btnSave_UpdateLoanRepaymentForm)


        // value holder view
        tvDatePicker = view.findViewById(R.id.tvDatePicker_UpdateLoanRepaymentForm)
        val etAmountRepaid = view.findViewById<EditText>(R.id.etAmountRepaid_UpdateLoanRepaymentForm)
        val etTransactionId = view.findViewById<EditText>(R.id.etTransactionId_UpdateLoanRepaymentForm)
        val etShortNotes = view.findViewById<EditText>(R.id.etShortNotes_UpdateLoanRepaymentForm)
        val actvSelectLender = view.findViewById<AutoCompleteTextView>(R.id.actvLoanRecipient_UpdateLoanRepaymentForm)
        val actvSelectPaymentMethod = view.findViewById<AutoCompleteTextView>(R.id.actvPaymentMethod_UpdateLoanRepaymentForm)
        ...
        val theLoanRepaymentId = arguments?.getInt("loanRepaymentId")
        val theLoanRepaymentDate = arguments?.getString("loanRepaymentDate")
        val theLender = arguments?.getString("lender")
        val thePaymentMethod = arguments?.getString("paymentMethod")
        val theAmountRepaid = arguments?.getString("amountRepaid")
        ...
        tvDatePicker.text = theLoanRepaymentDate
        etAmountRepaid.setText(theAmountRepaid)
        etTransactionId.setText(theTransactionId)
        actvSelectLender.setText(theLender)
        actvSelectPaymentMethod.setText(thePaymentMethod)
        etShortNotes.setText(theShortNotes)

        // save loan repayment info into the database
        save.setOnClickListener {
            if (theLoanRepaymentId != null) {
                insertDataToDatabase(
                    theLoanRepaymentId.toInt(),
                    tvDatePicker,
                    tvEnterDate,
                    tvSelectLender,
                    actvSelectLender,
    ...
    private fun insertDataToDatabase(
        ...
        if (checkIfEmpty(date)) {
            ...
            val loanRepaymentDate = newDate?.time?.let { java.sql.Date(it)}

            if (loanRepaymentDate != null) {
                mLoanRepaymentViewModel.updateLoanRepayment(loanRepaymentId,loanRepaymentDate, lender, paymentMethod, amountRepaid.toDouble(), transactionId, shortNotes)
            }
